...
const IMAGES_EXTENSIONS = [
  ...
  function logFailedToMatchMarkupWarning(input: string, debugOn: boolean) {
    warn(debugOn, `Failed to match markup '${input}'.`)
  }
  
  function logIncorrectMatchedMarkupWarning(input: string, src: string, path: string, debugOn: boolean) {
